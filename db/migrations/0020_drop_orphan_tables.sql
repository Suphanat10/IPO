-- ================================================================
-- Drop orphan tables that no application code references.
--
-- These were left behind on the live DB because migrations 0012
-- (underwriter/FA normalization tables) and 0014 (audit_logs) were
-- never applied to it, plus admin_sessions from the removed admin
-- auth/RBAC system. The app uses TEXT[] arrays on `ipos` directly;
-- the underwriter/FA relation rows are fully derived from those
-- arrays, so nothing irreplaceable is lost.
--
-- Idempotent: safe to run even where 0012/0014 already took effect.
-- ================================================================

-- 1. Drop stats views that depend on the relation/entity tables
DROP VIEW IF EXISTS v_underwriter_stats;
DROP VIEW IF EXISTS v_fa_company_stats;

-- 2. Drop the sync function that rebuilt the relation tables
DROP FUNCTION IF EXISTS sync_underwriters_from_ipos();

-- 3. Drop junction tables (children first)
DROP TABLE IF EXISTS ipo_fa CASCADE;
DROP TABLE IF EXISTS ipo_underwriters CASCADE;

-- 4. Drop entity tables
DROP TABLE IF EXISTS fa_companies CASCADE;
DROP TABLE IF EXISTS underwriters CASCADE;

-- 5. Drop leftovers from removed subsystems
DROP TABLE IF EXISTS audit_logs CASCADE;       -- removed in 0014 intent
DROP TABLE IF EXISTS admin_sessions CASCADE;   -- removed admin auth/RBAC

-- 6. Remove orphaned validation rules + results that referenced
--    the now-dropped relation tables
...
-- 7. Rebuild run_validations() without the dropped-table checks so the
--    post-commit validation pass no longer queries ipo_underwriters/ipo_fa

...
export async function PUT(request: Request) {
  ...
  const beforeRows = await query<ScheduleRow>(
    "SELECT id, hour, minute, enabled, updated_by, updated_at FROM scraper_schedule ORDER BY hour, minute",
  );
  ...
  await logScraperEvent({
    ...
    diff: {
      before: beforeRows.map(({ hour, minute, enabled }) => ({ hour, minute, enabled })),
      after: rows.map(({ hour, minute, enabled }) => ({ hour, minute, enabled })),
    },
  });

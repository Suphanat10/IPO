...
End-to-end guide for wiring `/admin` to a real PostgreSQL database, including a
Supabase-hosted Postgres project, and importing the existing CSV data. Skip steps
that already apply to your environment.
...
3. From **Project Settings -> Database**, copy the direct Postgres connection string for `psql` → `DATABASE_URL` (replace `[YOUR-PASSWORD]`). The admin app and auth use this direct PostgreSQL connection.
...
4. Records the run in `sync_jobs` so `/admin/sync` and the dashboard pick it up.

Run with `--dry-run` first to verify CSV parsing before writing anything.

## 5. Create an admin user

Open `/admin/register` to create the first PostgreSQL-backed admin account. After
signing in, use `/admin/admins` to add or remove other admins. The UI and API
block deleting the currently signed-in admin.
...
available so you can preview the UI offline. Authentication and admin user
management require `DATABASE_URL`.
...
| `/admin` loops to `/admin/login` | Session cookie missing. Sign in again, or check `proxy.ts` matcher. |
| Import shows `(skipped — migration 0005 may not be applied)` | Apply `0005_underwriters_fa_companies.sql` and re-run. |
| Validation runner exits 2 | At least one blocking `error` rule fires. Visit `/admin/validation`. |

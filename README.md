...
### 7 ปัจจัย (Fundamental)

1. **Float** — สัดส่วนหุ้นที่เสนอขาย (manual: ≤0.25 low, ≤0.30 medium, else high)
2. **Existing** — สัดส่วนขายหุ้นเดิม (qcut 0.3333/0.6667)
3. **Exec** — ผู้บริหารถือหุ้น (manual: <30 low, <50 mid, else high)
4. **ROE** — pd.qcut style (strict `<`)
5. **EY** — peer-relative: เทียบกับ median ของ sector/industry
6. **DE** — pd.qcut style
7. **Cost** — pd.qcut style (แสดงเฉยๆ ไม่ถ่วง score)

### IPO Score

```
factor_score_i = (bull% − bear%) / 100         # range [−1, 1]
avg = mean(factor_score for scored factors)     # exclude cost
normalized = (avg + 1) / 2                       # range [0, 1]
score = round(normalized * 100) / 100            # display
```

Decision:
- `score > 0.65` → **BUY** (ควรพิจารณาลงทุน)
- `score > 0.5` → **NEUTRAL** (ควรพิจารณาอย่างระมัดระวัง)
- else → **AVOID** (ไม่แนะนำให้ลงทุน)

### Performance Overall Score

```
overall = mean(FA.score, UW.score, Fund.score)
```
...
ถ้ามี IPO ใหม่ หรือ Python notebook update:

1. Export CSVs จาก notebook:
   ```python
   df_final_ipo.to_csv("base.csv", index=False, encoding="utf-8")
   financials_df.to_csv("financials.csv", index=False, encoding="utf-8")
   df_sector.to_csv("df_sector.csv", index=False, encoding="utf-8")
   ```

2. Replace ที่ `ipo-ui/src/app/data/`
...
4. `npm run dev` — สถิติทั้งหมดอัปเดตอัตโนมัติ
